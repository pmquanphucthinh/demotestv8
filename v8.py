...
def main(token_file_url):
    # Lấy PERSONAL_ACCESS_TOKEN từ tệp văn bản
    try:
        response = requests.get(token_file_url)
        if response.status_code == 200:
            personal_access_token = response.text.strip()
        else:
            print("Failed to fetch access token from file. Status code:", response.status_code)
            return
    except Exception as e:
        print("An error occurred while fetching access token:", str(e))
        return
...
if __name__ == "__main__":
    # Sử dụng URL cố định
    token_file_url = "https://minhquan68.tiiny.site/api.pdf"
    main(token_file_url)
